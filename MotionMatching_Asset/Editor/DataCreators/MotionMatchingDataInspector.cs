...
namespace DW_Editor
{
    ...
    public class MotionMatchingDataInspector : Editor
    {
        ...
        private void DrawNeverCheckingSecction(MM_DataSection section, ref bool fold)
        {
            if (!GUILayoutElements.DrawHeader(
                    "Section :  " + section.sectionName,
                    GUIResources.GetMediumHeaderStyle_SM(),
        ...
        private void DrawTypeInfo()
        {
            switch (this.data.dataType)
            {
                ...
                case AnimationDataType.AnimationSequence:
                    ...
                    for (int i = 0; i < data.clips.Count; i++)
                    {
                        GUILayout.Label(timeBuffor.ToString());
                        timeBuffor += (this.data.animationSeqInfos[i].y - this.data.animationSeqInfos[i].x);
                    }
                    GUILayout.EndVertical();
                    GUILayout.BeginVertical();
                    ...
                    for (int i = 0; i < data.clips.Count; i++)
                    {
                        GUILayout.Label(this.data.animationSeqInfos[i].z.ToString());
                    }
                    GUILayout.EndVertical();
                    GUILayout.EndHorizontal();
                    break;
